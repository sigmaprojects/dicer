    ...
    <v-container fluid>
      ...
      <div class="text-center mt-1">
        <v-sheet v-if="hitdice.length > 0 || totalDmg.length > 0">
          <span class="mr-2 text--red" style="word-spacing: 20px;">{{hitdice}}</span>
          <span class="ml-2">{{totalDmg}}</span>
        </v-sheet>
      </div>
    </v-container>
  </div>
...
export default {
  ...
  methods: {
    refreshsheet() {
      ...
      let elms = this.$el.querySelectorAll(
        "input[id^='checkbox'][checked=true], input[id^='checkbox']:checked"
      );
    ...
    updateResults() {
      ...
      var diceCount = {};
      skillDice.forEach(function(i) {
        diceCount[i] = (diceCount[i] || 0) + 1;
      });
